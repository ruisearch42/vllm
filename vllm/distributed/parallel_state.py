...
def init_distributed_environment(
    ...
    if config is not None and config.parallel_config.data_parallel_size > 1:
        ...
        logger.info(f"data_parallel_size: {parallel_config.data_parallel_size}, world_size_across_dp: {world_size}")
    ...
    if not torch.distributed.is_initialized():
        ...
        logger.info(f"init_process_group: backend={backend}, " 
    ...
    if _WORLD is None:
        ...
        logger.info(f"init_world_group: ranks={ranks}, local_rank={local_rank}, backend={backend}")
